...
def _band_tags(d: dict) -> list[str]:
    ...
    판정이 그 밴드 근처인지를 보므로 밴드 위인지 아래인지가 보여야 한다.
    """
    tags = []
    if d.get("trend"):
...
# 배열 태그 앞의 글리프 — **이평선이 어떻게 쌓였는지를 그린 것**이다.
# 정배열은 짧은 선이 위(120>240>480), 역배열은 그 반대, 혼조는 엇갈림.
#
# 🟩🟨🟥을 먼저 써 봤다가 뺐다: 큰 색 블록이 줄마다 하나씩 박혀 정작 중요한
# 마크(🍃·↗️·🪜)보다 눈에 먼저 걸렸고, 무엇보다 **초록이 '좋다'로 읽히는데
# 뜻은 정반대**였다 — 정배열은 이미 오를 만큼 오른 자리라 셋 중 제일 나쁘다
# (실측 승률 57.6% · 쪽박 22%, 혼조는 67.2% · 16%). 색은 서열을 암시하는데
# 그 서열이 틀렸다. 화살표는 **배치를 그릴 뿐 좋고 나쁨을 말하지 않는다.**
#
# 이모지가 아닌 맨 글리프라 ↗️(저항 테스트)와 무게가 확실히 다르다 —
# 같은 화살표라도 줄에서 튀지 않는다.
...
def _resist_tags(d: dict) -> list[str]:
    """1h·15m 저항 테스트 표시 — 큰 눈금부터. 둘 다 붙을 수 있다."""
    out = []
    for key, tags in (("resist_1h", RESIST_TAGS), ("resist_15m", RESIST15_TAGS)):
        r = d.get(key)
        if isinstance(r, str) and r in tags:
            out.append(tags[r])
    return out
...
#  둘 중 하나를 바꿔야 한다.)
EARLY_TAG = "🌱상승초기"
# 🪜회복구간 — 4h 120선 < 480선(아직 하락 구조)인데 캔들이 그 사이. 바닥에서
# 올라와 120선은 되찾았고 480선이 남은 자리다(leader_break.recovery_band).
# **배열 태그와 같이 읽어야 한다** — 같은 🪜인데 1h 혼조면 경로승률 70%,
# 정배열이면 56%로 갈린다. 그래서 거르지 않고 나란히 둔다.
BAND_TAG = "🪜회복구간"
