...
class Params:
    ...
    # 🪜회복구간 — 4h 120선 < 480선(아직 하락 구조)인데 캔들이 **그 사이**에
    # 있는 상태. 바닥에서 올라와 120선은 되찾았고 480선이 남은 자리다.
    # **거르지 않고 표시만 한다**(quiet과 같은 성격).
    band_enabled: bool = True
    band_mas: tuple = (120, 480)     # 4h 프레임 기준 (20일 · 80일)
...
def blocked(df1h: pd.DataFrame, params: Params = Params(),
    ...
    ① **통과시킬 배열을 고른다**(require_aligned). 기본은 정배열만이었고,
       allow_mixed·allow_bearish로 혼조·역배열을 각각 열 수 있다.
       require_aligned를 끄면 배열 판정 자체를 안 본다.

       **원래 전제는 실측에서 뒤집혔다.** "24h 상승률 상위인데 1h가
       역배열·혼조면 하락 추세 안의 반등이라 눌림으로 볼 자리가 아니다"라고
       적어 놨었는데, 31일 리플레이(현물 미러 370종, 15m 20선 이탈 10,651건,
       이탈 봉 종가 진입·전진 7일)에서 정배열이 세 버킷 중 **꼴찌**였다:

           배열     경로승률   MFE     MAE     대박    쪽박
           정배열    57.6%   12.4%   −8.9%   35%    22%
           혼조      67.2%   14.1%   −7.7%   38%    16%
           역배열    65.8%   16.1%   −6.8%   43%    18%

       정배열은 이미 오를 만큼 오른 자리라 20선 이탈이 '눌림'이 아니라
       '첫 균열'인 경우가 많다(쪽박률이 셋 중 제일 높다). 혼조·역배열은
       깊은 데서 올라오는 중이라 더 빠질 여지가 적고 반등폭이 크다.

       그래서 **allow_mixed를 켰다** — 정배열 + 혼조. 알림은 하루 328 →
       566건(퍼프 환산)으로 늘고 경로 승률은 57.8 → 61.8%, 쪽박률은
       20 → 18%가 된다. allow_bearish까지 켜면 712건/일·62.7%인데,
       ⚡가 이미 전체 알림의 88%라 거기까지는 안 갔다.

       **단, 이득은 "감시 이월분"에서만 나온다.** 지금 이 순간 24h 상승률
       TOP10인 종목만 따로 보면 세 배열이 평평했다(51.8/51.0/49.7%).
       열흘 전 상위였다가 밀린 종목의 되돌림이 깊어서 생기는 차이다.
       국면 의존도 크다 — 전반 12일 vs 후반 12일로 쪼개면 혼조 승률이
       59.8% → 72.5%로 흔들린다(방향은 양쪽 다 같다).
    ② **정배열일 때만 4h 장기 수퍼트렌드가 하락이면 제외**
       (exhausted_filter). 캔들이 4h 장기선 **위**에 있어야 통과다. 큰
       추세가 아직 안 꺾였다는 확인이고, 꺾였으면 오를 만큼 오른 뒤
       돌아선 것이라 새로 잡을 눌림이 아니다. 실측에서도 정배열·수트↓가
       쪽박률 33%로 전 버킷 최악이라 이건 제대로 자르고 있다.

       **정배열에만 건다.** 역배열은 장기선 아래인 게 정상이라 걸면 통째로
       다시 잘리고, 혼조는 걸면 오히려 나빠진다 — 혼조·수트↓(70.9%)가
...
def recovery_band(df4h: pd.DataFrame | None,
    ...
    """🪜회복구간인지 — 4h 120선 < 480선인데 종가가 그 사이.

    **거르지 않고 표시만 한다.** 120선(20일)이 480선(80일) 아래면 큰 눈금은
    아직 하락 구조인데, 그 안에서 종가가 120선 위로 올라와 480선을 향하는
    자리다 — 바닥에서 올라와 마지막 저항대를 오르는 중이라는 뜻이다.

    **1h 배열과 같이 읽어야 한다.** 31일 리플레이(top_n 10 · 게이트 ·
    ↗️1h 요구, 2,821건)에서 같은 🪜인데 1h 배열로 정반대로 갈렸다:

        🪜 · 1h 혼조   425건  경로승률 69.9%  MFE 11.9%  대박 33%  쪽박  7%
        🪜 · 1h 정배열 312건  경로승률 56.2%  MFE 10.8%  대박 31%  쪽박 18%
    ...
    짧은 선은 240에서 120으로 넓혔다 — 240~480은 거의 전부 120~480 안에
    들어가고(240에만 걸리는 건 2,821건 중 9건), 붙는 비중이 16% → 26%가
    되는 대신 갈림이 19.4%p → 13.7%p로 조금 무뎌진다. 방향은 그대로다.
