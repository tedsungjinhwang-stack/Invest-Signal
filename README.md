...
(AKE·ESPORTS 사례). 펌핑 코인의 첫 눌림 진입 자리를 주간 VWAP로 잡는다:

1. 최근 `lookback_bars`(기본 42봉 = 7일) 안에서 **저점 대비 고점이
   `min_gain`(기본 +100%) 이상** 펌핑했고
2. 펌핑 고점 이후 캔들(저가)이 **주간 앵커드 VWAP(WVWAP) 라인에 처음
   닿는 봉**에서 🚀 알림

고점 이후 첫 터치만 알리고, 새 고점 뒤 다시 닿으면 새 셋업으로 재알림.
추적은 종가가 주간 VWAP 위에서 버티는 동안 유지 — 아래로 무너지면 제거.
