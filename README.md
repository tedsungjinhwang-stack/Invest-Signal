...
- **📌 유지 중**: 추적 중인 셋업 목록 — `심볼(경과일d·배열)`
  - 상승초입: 종가가 계속 60선 아래인 동안 (진입 존 열려 있음)
  - 눌림목: 발생 후 계속 표시 — 120선 위로 복귀해 정배열로 올라가는 동안도
    추적하며, 하락전환(CHoCH)이 나와야 제거된다
  - MSS: 종가가 계속 깨진 직전저점 아래인 동안 (경고 마커로 병행 표시)
  - 최근 7일(42봉) 내 트리거만 조회, 시그널당 최신 15종목까지 표시
- **하락전환은 표시하지 않음** — 셋업 이후 하락전환이 발생한 종목의
  상승초입·눌림목 항목을 유지 중 리스트에서 영구 제거하는 역할만 한다
