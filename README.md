...
**인트라바 스캔**: 터치는 봉 진행 중에도 확정되는 사실이라, 봉 중간
(마감 2시간 뒤, `scan-intrabar.yml`)에 진행 중 봉 포함으로 펌핑만 한 번
더 스캔한다 — 봉 초반 터치는 마감을 기다리지 않고 최대 ~2시간 내 알림.
종가 조건 시그널(상승초입·눌림목·하락전환·MSS)은 마감 스캔에서만 판정.
...
03:10/07:10/11:10/15:10/19:10/23:10)에는 `scan-intrabar.yml`이 펌핑
인트라바 터치만 스캔해 사실상 **2시간 간격**이 된다. (GitHub 크론
특성상 수십 분 지연될 수 있음)
