...
무너지면 제거. 크립토 랭크 필터(눌림목과 동일 기준)를 통과해야 한다.

**인트라바 스캔**: 봉 중간(마감 2시간 뒤, `scan-intrabar.yml`)에 진행 중
봉을 포함해 한 번 더 스캔한다 — 펌핑 터치(봉 진행 중에도 확정되는
사실)와 상승초입·눌림목(현재가를 잠정 종가로 판정, 알림에 **⏳진행봉**
태그)이 대상. ⏳진행봉 알림은 봉 마감 때 조건이 풀리면 무효일 수 있고,
마감 후에도 유지되면 그대로 확정이다(같은 봉이라 재알림 없음).
하락전환·MSS는 마감 스캔에서만 판정.
...
- **↳ 추적 리스트**: 각 시그널·시장 칸 아래 한 줄 — `심볼(경과일d·배열)`
  ...
  - 눌림목: 발생 후 계속 표시 — 120선 위로 복귀해 올라가는 동안도 추적하며,
    **하락전환(CHoCH)이 나오거나 종가가 분기 VWAP 아래로 마감하면
    제거**된다
