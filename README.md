...
3. **60선 + 분기 VWAP 이탈** — 터치 후 `touch_window_bars`(기본 60봉 =
   10일) 이내에 종가가 60선 아래이면서 **분기 앵커드 VWAP(QVWAP)도 깨고
   내려온 첫 봉**에서 알림. QVWAP 위에 있다가 아래로 확 내려오는 플러시를
   잡는다 — 60선만 깨고 QVWAP 위에서 버티는 동안은 대기
   (`qvwap_condition: false`면 60선 이탈만 본다)
4. **크립토 랭크 필터 (완화판)** — 3파가 나오기 전 초입 단계라 풀백보다
   기준을 낮게: 24h 거래대금 상위 `volume_top`(기본 200) 또는 최근 7일
...
3. **크립토 랭크 필터** — 크립토는 주도주만: 24h 거래대금 상위
   ...
   풀백 칸에 표시되는 크립토 **MSS 줄에도 동일하게 적용**된다.
   ETF·주식에는 적용하지 않는다
...
**ETF·주식만 🔻하락전환 칸에 표시**된다. 크립토는 표시하지 않고, 풀백
...
- **신규 시그널**: 시그널별(🟢상승초입 🔵풀백 🔻하락전환 — MSS는 풀백 칸에
...
- **크립토 하락전환은 표시하지 않음** — 셋업 이후 하락전환이 발생한 종목의
  풀백 항목을 추적 리스트에서 영구 제거하는 역할만 한다
