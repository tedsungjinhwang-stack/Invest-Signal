...
4. **크립토 랭크 필터 (주도주)** — 크립토는 **그 시기의 주도주만**:
   ...
   최근 30일(`gain_lookback_bars`, 180봉) 상승률 상위
   ...
   **MSS 줄에도 동일하게 적용**된다. ETF·주식에는 적용하지 않는다
   ...
   > 동일). 둘 다 요구하고 상승률 창을 3일→30일로 넓히면 1.7건/일·도달률 61%로
   > 올라간다. 상승률 조건이 선별력의 대부분을 만들고 거래대금 순위는 $5M
   > 하한과 사실상 중복이라, 하한만 $20M 이상으로 올리면 오히려 대형주만 남아
   > 도달률이 41%로 떨어진다.
